...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("created_at", MySQLDateTime(fsp=3), nullable=False,
                  server_default=sa.text("CURRENT_TIMESTAMP(3)")),
        sa.Column("updated_at", MySQLDateTime(fsp=3), nullable=False,
    ...
    op.create_table(
        ...
        sa.Column("last_message_at", MySQLDateTime(fsp=3), nullable=True),
        sa.Column("created_at", MySQLDateTime(fsp=3), nullable=False,
                  server_default=sa.text("CURRENT_TIMESTAMP(3)")),
        sa.Column("updated_at", MySQLDateTime(fsp=3), nullable=False,
                  server_default=sa.text("CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)")),
    ...
    op.create_table(
        ...
        sa.Column("started_at", MySQLDateTime(fsp=3), nullable=True),
        sa.Column("finished_at", MySQLDateTime(fsp=3), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("expires_at", MySQLDateTime(fsp=3), nullable=False, comment="签发后 60 秒"),
        sa.Column("consumed_at", MySQLDateTime(fsp=3), nullable=True, comment="一次性消费标记"),
        sa.Column("created_at", MySQLDateTime(fsp=3), nullable=False,
                  server_default=sa.text("CURRENT_TIMESTAMP(3)")),
        sa.PrimaryKeyConstraint("id"),
