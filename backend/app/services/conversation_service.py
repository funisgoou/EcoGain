...
async def delete_many(session: AsyncSession, user_id: int, conversation_ids: list[int]) -> int:
    ...
    → 会话行软删 → 提交 → 磁盘与 DuckDB 表清理（失败仅记日志不回滚）。
    实现说明：DB 变更走自管事务（new_session），而非请求事务——AsyncSession 不支持
    after_commit 等异步事件监听（曾因此 500），且清理动作必须在 commit 之后执行。
    """
    ...
    # 3) 磁盘与 DuckDB 清理（DB 已提交；失败仅记日志不回滚）
    duck = get_duckdb()
